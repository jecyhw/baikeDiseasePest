...
$(document).ready(function () {
    var id = "", $picture_form = $("#picture_form"), $info_form = $("#info_form"), preview = $('.preview'), progress, filename, uploader, type = $(document.body).attr('type');
    $("#info_submit").click(function () {
        ...
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: $('input[type=text], textarea').serializeArray(),
            beforeSend: function () {
    ...
    $('#info_reset').click(function () {
        $info_form.find(":input").val('');
        $picture_form.find(":input").val('');
        $("#pictures").html('');
    ...
    $('#info_del').click(function () {
        if (id.length > 0) {
            $.get(addBasePath(type + '/delete/' + id), function (data) {
    ...
    uploader = new plupload.Uploader({
        ...
        flash_swf_url : addBasePath('plupload/Moxie.swf'),
        silverlight_xap_url : addBasePath('plupload/Moxie.xap'),
        ...
        init: {
            PostInit: function() {
                ...
                $('#picture_submit').click(function() {
                    if (filename.find('div').attr('id')) {
                        ...
                        filename.html('<div class="alert alert-danger">请先选择文件再上传</div>'), progress.hide();
                    }
                    return false;
                });
            },
            ...
            BeforeUpload: function (up, files) {
                $picture_form.find(":input").prop("disabled", true);
            ...
            Error: function(up, err) {
                ...
                $picture_form.find(":input").val('').prop("disabled", false);
            },
    ...
    function modeSwitch() {
        ...
        //是否启用上传表单
        $picture_form.find(":input").prop("disabled", id.length ? false : true);
    ...
    function createColumnByPicture(picture) {
        ...
        del.click(function () {
            //先删除虫害记录里的图片
            $.get(addBasePath(type + '/delete/' + id + '/picture/' + picture.fileId), function (data) {
                if (data.code == 200) {
                    pageAlert(data.message, 'success');
                    //再删除图片文件
                    $.get(addBasePath('picture/delete/' + picture.fileId), function (data) {
                        pageAlert(data.message, 'success');
